    ...
    <el-card>
        <template #header>
            <div class="card-header">
                <span style="margin-top: auto; margin-bottom: auto;">DNS List</span>
        ...
        >
            ...
            >
                <template #default="scope">
                    {{ scope.row.ttl === 1 ? 'automatic' : scope.row.ttl }}
                </template>
            </el-table-column>
            ...
            >
                <template #default="scope">
                    <el-popconfirm
                        ...
                        cancel-button-text="Forget It"
                        icon="el-icon-info"
                        icon-color="red"
                        title="Do you really want to delete this record? It will disappear permanently (really long)"
...
async function changeProxied(record: CloudflareDnsRecord, proxied: boolean) {
    ...
    if (!res) {
        ElMessage('Modification is successful and is refreshing')
        await refresh()
    } else {
        ElMessage({
            type: 'error',
            message: `Modification failed, wrong: ${res}`,
        })
    }
}
...
async function doDeleteRecord(record: CloudflareDnsRecord) {
    ...
    if (deleteError) {
        ElMessage({
            type: 'error',
            message: `Delete failure, the reason: ${deleteError}`,
        })
    } else {
        ElMessage('Delete successfully, refresh')
    }
